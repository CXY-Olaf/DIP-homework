...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch, num_epochs, sample_every):
    """
    ...
    for i, (image_rgb, image_semantic) in enumerate(dataloader):
        ...
        if sample_every > 0 and epoch % sample_every == 0 and i == 0:
...
def validate(model, dataloader, criterion, device, epoch, num_epochs, sample_every):
    """
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Pix2Pix-style FCN on facades.")
...
def main():
    ...
    print(
        f"Training on {device} with batch_size={args.batch_size}, "
        f"num_workers={args.num_workers}, epochs={num_epochs}, start_epoch={args.start_epoch}"
    )
    for epoch in range(args.start_epoch, num_epochs):
        train_one_epoch(
            model, train_loader, optimizer, criterion, device, epoch, num_epochs, args.sample_every
        )
        validate(
            model, val_loader, criterion, device, epoch, num_epochs, args.sample_every
        )
