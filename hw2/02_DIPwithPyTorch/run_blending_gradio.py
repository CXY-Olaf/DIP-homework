...
def add_point(img_original, polygon_state, evt: gr.SelectData):
    ...
    if len(polygon_state['points']) > 1:
        draw.line(polygon_state['points'], fill='red', width=LINE_WIDTH)
...
def close_polygon(img_original, polygon_state):
    ...
    if not polygon_state['closed'] and len(polygon_state['points']) > 2:
        ...
        img_with_poly = img_original.copy()
        draw = ImageDraw.Draw(img_with_poly)
        draw.polygon(polygon_state['points'], outline='red', width=LINE_WIDTH)
        for point in polygon_state['points']:
            draw.ellipse(
                (
                    point[0] - POINT_RADIUS,
...
def update_background(background_image_original, polygon_state, dx, dy):
    ...
    if polygon_state['closed']:
        ...
        draw.polygon(shifted_points, outline='red', width=LINE_WIDTH)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    if foreground_values.numel() != blended_values.numel():
        valid_length = min(foreground_values.numel(), blended_values.numel())
        foreground_values = foreground_values[:valid_length]
        blended_values = blended_values[:valid_length]

    loss = F.mse_loss(blended_values, foreground_values)
...
def load_foreground_image(img_original):
    return img_original, img_original, initialize_polygon()


def reset_polygon_state(img_original, background_image_original):
    return img_original, initialize_polygon(), background_image_original, gr.update(value=0), gr.update(value=0)
...
""") as demo:
    ...
    foreground_image_original_state = gr.State(value=None)
    ...
    with gr.Row():
        with gr.Column():
            ...
            foreground_image_original = gr.Image(
                label="", type="pil", interactive=True, height=380
            )
            gr.Markdown(
            ...
            foreground_image_with_polygon = gr.Image(
                label="Click in this lower panel to add polygon points", type="pil", interactive=False, height=380
            )
            gr.Markdown(
                "<p style='font-size: 0.9em;'>Click on the lower image to define the polygon area. Blue points and red edges should appear immediately. After selecting at least three points, click <strong>Close Polygon</strong>.</p>"
            )
            with gr.Row():
                close_polygon_button = gr.Button("Close Polygon")
                reset_polygon_button = gr.Button("Reset Polygon")
        with gr.Column():
    ...
    with gr.Row():
        with gr.Column():
            ...
            background_image_with_polygon = gr.Image(
                label="", type="pil", interactive=False, height=520
            )
        ...
        with gr.Column():
            ...
            output_image = gr.Image(
                label="", type="pil", interactive=False, height=520  # Increased height for larger display
            )
    ...
    foreground_image_original.change(
        ...
        outputs=[foreground_image_with_polygon, foreground_image_original_state, polygon_state],
        queue=False,
    ...
    foreground_image_with_polygon.select(
        add_point,
        inputs=[foreground_image_original_state, polygon_state],
    ...
    close_polygon_button.click(
        ...
        inputs=[foreground_image_original_state, polygon_state, dx, dy, background_image_original],
        outputs=[foreground_image_with_polygon, polygon_state, background_image_with_polygon, dx],
        queue=False,
        show_progress="hidden",
    )

    reset_polygon_button.click(
        fn=reset_polygon_state,
        inputs=[foreground_image_original_state, background_image_original],
