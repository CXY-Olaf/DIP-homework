...
def _farthest_point_sample(points: torch.Tensor, k: int) -> torch.Tensor:
    """Greedy farthest-point sampling. Runs on the same device as `points`."""
    N = points.shape[0]
    if k >= N:
        return torch.arange(N)
    # GPU is ~100x faster for the inner loop when available.
    device = points.device
    pts = points.cuda() if torch.cuda.is_available() else points
    selected = torch.empty(k, dtype=torch.long, device=pts.device)
    dist = torch.full((pts.shape[0],), float('inf'), device=pts.device)
    selected[0] = 0
    for i in range(1, k):
    ...
    return selected.cpu() if device.type == 'cpu' else selected
...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Down-sample sparse points with farthest-point sampling so that the
        # renderer's O(N·H·W) memory stays within VRAM. Pure-torch FPS keeps
        # spatial coverage much better than uniform random sampling.
