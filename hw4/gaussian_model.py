...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        # Compute mean distance to K nearest neighbors (torch-native replacement
        # for pytorch3d.knn_points to avoid the heavy build dependency).
        K = min(50, self.n_points - 1)
        pts = torch.as_tensor(points3D_xyz, dtype=torch.float32)  # (N, 3)
        d = torch.cdist(pts, pts)                                  # (N, N) euclidean
        knn_d, _ = torch.topk(d, K, dim=-1, largest=False)         # (N, K) includes self=0
